...
export default function UniversityPostsScreen() {
    ...
    const [showNewsDatePicker, setShowNewsDatePicker] = useState(false);
    ...
    }, [user, pageNews, hasMoreNews, loadingNews, filterDate, filterStartTime, filterEndTime, filterCategory, sortBy, sortOrder]);
    ...
    }, [user, pageNews, hasMoreNews, loadingNews, keyword, filterDate, filterStartTime, filterEndTime, filterCategory, sortBy, sortOrder]);
    ...
    const handleCreateNews = async () => {
        ...
        try {
            const res = await fetchWithoutAuth(`/universityNews`, {
                method: "POST",
            ...
            alert("Something went wrong while creating the news.");
        } finally {
            ...
            <GestureHandlerRootView style={styles.appContainer}>
                    ...
                    <BottomSheetView>
                        ...
                        >
                            <View style={{ gap: 15 }}>
                                ...
                                <View>
                                    <TouchableOpacity onPress={() => { applyFilters() }} style={styles.modalButton} disabled={filteringNews}>
                                        <Text style={styles.modalButtonText}>Apply filters</Text>
                                        {filteringNews && <ActivityIndicator size='small' color={'#fff'} />}
                                    </TouchableOpacity>
                                </View>
                            </View>
                        </BottomSheetScrollView>
                    ...
                    <BottomSheetView>
                        ...
                        >
                            <View style={{ gap: 15 }}>
                                ...
                                <View>
                                    <TouchableOpacity onPress={() => { applySorting() }} style={styles.modalButton} disabled={sortingNews}>
                                        <Text style={styles.modalButtonText}>Sort</Text>
                                        {sortingNews && <ActivityIndicator size='small' color={'#fff'} />}
                                    </TouchableOpacity>
                                </View>
                            </View>
                        </BottomSheetScrollView>
                ...
                >
                    <View style={[styles.modalHeader, { paddingTop: 10, paddingBottom: 10 }]}>
                        <Text style={styles.modalTitle}>Create a news</Text>
                    ...
                    >
                        ...
                        <View style={{ marginBottom: 15 }}>
                            ...
                            <TouchableOpacity
                                style={[styles.filterInput, { justifyContent: 'center' }]}
                                onPress={() => {
                                    setShowNewsDatePicker(true)
                                    if (Platform.OS == 'ios') { createRef.current?.expand() }
                                }}
                            >
                                <Text style={{ color: newsDate ? (colorScheme === 'dark' ? '#fff' : '#000') : '#aaa' }}>
                                    {newsDate || 'Select Date'}
                                </Text>

                                {showNewsDatePicker && (
                                    <DateTimePicker
                                        value={newsDate ? new Date(newsDate) : new Date()}
                                        mode="date"
                                        display={Platform.OS === 'ios' ? 'inline' : 'default'}
                                        onChange={(event, selectedDate) => {
                                            setShowNewsDatePicker(false); // keep open for iOS inline
                                            if (selectedDate) setNewsDate(selectedDate.toISOString().split('T')[0]);
                                        }}
                                    />
                                )}
                            </TouchableOpacity>
                        ...
                        <View style={{ gap: 10, paddingTop: 20, marginBottom: 80 }}>
                            <TouchableOpacity onPress={() => { handleCreateNews() }} style={styles.modalButton} disabled={creatingNews}>
                                <Text style={styles.modalButtonText}>{creatingNews ? 'Creating' : 'Create'} News</Text>
                                {creatingNews && <ActivityIndicator size='small' color={'#fff'} />}
                            </TouchableOpacity>
                        </View>
                    </BottomSheetScrollView>
